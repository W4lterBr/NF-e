...
def extrair_dados_nfse(xml_path):
    ...
    try:
        ...
        if inf_nfse is None:
            print(f"     infNFSe no encontrado em {xml_path.name}")
            return None
        
        chave_id = inf_nfse.get('Id', '')
        if not chave_id:
            print(f"     Atributo Id no encontrado em {xml_path.name}")
            return None
        ...
        # Tomador (destinatrio) - estrutura: toma > CNPJ
        cnpj_tomador = tree.findtext('.//nfse:toma/nfse:CNPJ', namespaces=ns) or ''
        ie_tomador = ''  # NFSe nacional no tem IE do tomador
        ...
        # Natureza (descrio do servio) - estrutura: serv > cServ > xDescServ
        ...
        try:
            ...
            if 'xmls' in partes:
                ...
                informante = partes[idx_xmls + 1]  # CNPJ logo aps 'xmls'
            else:
...
def importar_nfse():
    ...
    if not base_dir.exists():
        print(" Pasta xmls/ no encontrada!")
        return
    ...
    for idx, xml_path in enumerate(xmls_nfse, 1):
        ...
        if cursor.fetchone():
            print(f"  J existe (chave: {dados['chave'][:20]}...)")
            duplicados += 1
            continue
        ...
        try:
            ...
            print(f"  Erro ao registrar XML: {e}")
        ...
        try:
            ...
            print(f" Importado (N {dados['numero']}, R$ {float(dados['valor']):,.2f})")
            importados += 1
    ...
    # Resumo
    print("\n" + "="*80)
    print(" RESUMO DA IMPORTAO")
    print("="*80)
    print(f" Importados com sucesso: {importados}")
    print(f"  J existiam no banco: {duplicados}")
    print(f" Erros: {erros}")
