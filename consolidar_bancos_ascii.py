...
def migrar_nfse_para_principal():
    ...
    if not Path(BANCO_TEST).exists():
        print(f" {BANCO_TEST} no encontrado!")
        return
    
    if not Path(BANCO_PRINCIPAL).exists():
        print(f" {BANCO_PRINCIPAL} no encontrado!")
        return
    ...
    try:
        ...
        if total_nfse == 0:
            print("  Nenhuma NFS-e para migrar")
            return
        ...
        print(f"    {xmls_duplicados} j existiam")
        ...
        print(f" Backup salvo antes da migrao")
        print("="*80 + "\n")
        
    except Exception as e:
        print(f"\n ERRO durante migrao: {e}")
        import traceback
        traceback.print_exc()
...
def listar_bancos_desnecessarios():
    """Lista bancos que podem ser deletados aps migrao"""
    print("\n" + "="*80)
    print("  BANCOS QUE PODEM SER DELETADOS")
    print("="*80)
