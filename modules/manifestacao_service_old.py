...
def validar_xml_evento_xsd(evento_xml, is_cte=False, tipo_evento=None):
    """
    Valida XML do evento contra o schema XSD oficial.
    
    Args:
        evento_xml: ElementTree do evento OU envEvento completo
        is_cte: True se for CT-e, False se for NF-e
        tipo_evento: Código do evento (210200, 210210, etc.) - não usado mais
        
    Returns:
        tuple (bool, str): (sucesso, mensagem_erro)
    """
    try:
        # ✅ Todos os eventos (incluindo manifestação) usam leiauteEvento_v1.00.xsd
        # O schema valida tanto <evento> quanto <envEvento>
        xsd_file = 'leiauteEvento_v1.00.xsd' if not is_cte else 'eventoCTe_v4.00.xsd'
        ...
        with open(xsd_path, 'rb') as f:
            # ⚠️ CRÍTICO: base_url permite resolver includes relativos (tiposBasico_v1.03.xsd, xmldsig-core-schema, etc.)
...
class ManifestacaoService:
    ...
    def __init__(self, cert_path, cert_password, db=None):
        ...
        logger.info("✅ Usando assinatura local com xmlsec (compatível SEFAZ)")
    ...
    def assinar_e_montar_soap(self, evento_root, cnpj_destinatario, tipo_evento, is_cte):
        ...
        valido, msg_validacao = validar_xml_evento_xsd(evento_root, is_cte, tipo_evento)
        if not valido:
            raise ValueError(f"XML inválido conforme XSD:\n{msg_validacao}")
        logger.info(f"  ✓ {msg_validacao}")
        
        # PASSO 1: NÃO limpar espaços - manter XML como construído
        logger.info("PASSO 1: Mantendo XML como construído (sem limpeza)")
        # ⚠️ REMOVIDO: Limpeza de whitespace pode quebrar o schema
        # O lxml já constrói corretamente sem espaços extras
        ...
        # DEBUG: Logar XML ANTES de assinar (para análise)
        xml_antes_assinar = etree.tostring(evento_root, encoding='unicode', pretty_print=False)
        logger.debug(f"  - XML ANTES de assinar ({len(xml_antes_assinar)} bytes):")
        logger.debug(xml_antes_assinar[:500] + "..." if len(xml_antes_assinar) > 500 else xml_antes_assinar)
        ...
        signature_node = xmlsec.template.create(
            evento_root,
            xmlsec.Transform.EXCL_C14N,  # 🔧 C14N Exclusive resolve erro 297 localmente
        ...
        # Transforms: ENVELOPED + C14N Exclusive
        ...
        try:
            ...
            # Serializa e re-parseia para simular o que a SEFAZ vai receber
            xml_str = etree.tostring(evento_root, encoding='utf-8', xml_declaration=False)
            evento_verificacao = etree.fromstring(xml_str)
            xmlsec.tree.add_ids(evento_verificacao, ["Id"])
            
            # Busca Signature no XML re-parseado
            signature_verif = evento_verificacao.find('.//{http://www.w3.org/2000/09/xmldsig#}Signature')
            
            if signature_verif is not None:
                ctx_verify.verify(signature_verif)
                logger.info("  ✅ Assinatura verificada: VÁLIDA")
            else:
                logger.warning("  ⚠️ Signature não encontrada após re-parse")
        except xmlsec.Error as e:
            logger.error(f"  ❌ Assinatura INVÁLIDA logo após assinar: {e}")
            logger.error("  💡 Problema possível: Certificado X509, transforms ou canonização")
        ...
        try:
            debug_path = Path(__file__).parent.parent / "debug_manifestacao.xml"
            with open(debug_path, 'w', encoding='utf-8') as f:
                f.write(soap_xml)
            logger.info(f"  📁 XML salvo para debug: {debug_path}")
        except Exception as e:
            logger.debug(f"  ⚠️ Erro ao salvar debug: {e}")
    ...
    def enviar_manifestacao(self, chave, tipo_evento, cnpj_destinatario, justificativa=None):
        ...
        try:
            ...
            # ============================================================================
            # ASSINATURA LOCAL XMLSEC (TESTADA E CORRIGIDA)
            # ============================================================================
            logger.info("=" * 80)
            logger.info("USANDO ASSINATURA LOCAL XMLSEC (TESTADA E CORRIGIDA)")
            logger.info("=" * 80)
            
            if is_cte:
                logger.info("CT-e detectado - usando assinatura xmlsec")
            else:
                logger.info("✅ NF-e - usando assinatura xmlsec com C14N Exclusivo")
            
            cuf = chave[0:2]
            
            # Monta descrição do evento (⚠️ EXATAMENTE como na NT 2020.001)
            desc_evento_map = {
                '210200': 'Confirmacao da Operacao',      # ✅ Confirmação da Operação
                '210210': 'Ciencia da Operacao',          # ✅ Ciência da Operação
                '210220': 'Desconhecimento da Operacao',  # ✅ Desconhecimento da Operação
                '210240': 'Operacao nao Realizada',       # ✅ Operação não Realizada
            ...
            if tipo_evento == '610110':  # Prestação em Desacordo (CT-e)
                ...
                det_evento = etree.SubElement(inf_evento, f"{{{ns}}}detEvento", versaoEvento="1.00")  # 🔧 CORREÇÃO: versaoEvento (não versao)
