...
# -*- coding: utf-8 -*-
"""
🧪 VERSÃO DE TESTE - AMBIENTE DE PRODUÇÃO (tpAmb=1) 🧪

Este arquivo é uma cópia de nfe_search.py configurada para:
- Usar ambiente de PRODUÇÃO (tpAmb=1) mas com dados isolados
- Conectar aos servidores da SEFAZ (mesmos de produção)
- Usar banco de dados separado (notas_test.db)
- Salvar XMLs em pasta separada (xmls_test/)

USO: Para testar logs HTTP e comunicação sem afetar banco de produção
Para produção normal, use nfe_search.py
"""
...
def salvar_xml_por_certificado(xml, cnpj_cpf, pasta_base="xmls_test", nome_certificado=None):  # ⚠️ PASTA DE TESTE
    """
    ⚠️ VERSÃO DE TESTE - Salva em xmls_test/ por padrão ⚠️
...
def validar_xml_auto(xml, default_xsd):
    ...
    return True
# -------------------------------------------------------------------
# URLs dos serviços - TESTE COM PRODUÇÃO
# -------------------------------------------------------------------
# 🧪 VERSÃO DE TESTE - USA tpAmb=1 (PRODUÇÃO) com dados isolados
# As URLs são as mesmas de produção. Dados salvos em banco/pasta separados.
URL_DISTRIBUICAO = (
...
CONSULTA_WSDL = {
    ...
    '52': "https://www.sefazvirtual.fazenda.gov.br/NFeConsultaProtocolo4/NFeConsultaProtocolo4.asmx?wsdl",  # GO
}
...
class NFeService:
    ...
    def fetch_by_cnpj(self, tipo, ult_nsu):
        ...
        etree.SubElement(distInt, "tpAmb").text    = "1"  # ✅ PRODUÇÃO (TESTE)
    ...
    def fetch_prot_nfe(self, chave):
        ...
        xml_consulta = f'''<consSitNFe xmlns="http://www.portalfiscal.inf.br/nfe" versao="4.00"><tpAmb>1</tpAmb><xServ>CONSULTAR</xServ><chNFe>{chave}</chNFe></consSitNFe>'''  # ✅ PRODUÇÃO (TESTE)
...
def run_single_cycle():
    """
    🧪 VERSÃO DE TESTE - PRODUÇÃO COM DADOS ISOLADOS 🧪
    ...
    - Usa notas_test.db ao invés de notas.db (dados isolados)
    - Salva XMLs em xmls_test/ ao invés de xmls/ (pasta separada)
    - Conecta aos servidores de produção (tpAmb=1) - documentos REAIS
    """
    data_dir = get_data_dir()
    db = DatabaseManager(data_dir / "notas_test.db")  # ⚠️ BANCO DE TESTE
    parser = XMLProcessor()
